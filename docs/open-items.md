...
- **Hosting choice** (from v0.1.0; first revisited in `docs/superpowers/specs/2026-06-24-drop-vercel-render-design.md`, which proposed Caddy + `tender-watch.galactix.co.za`; superseded again by an undocumented pivot — no superpowers spec covers this second move, see note below)
  - **Decision:** Single self-hosted origin at `watch.titan-ai.co.za`, on the
    shared xneelo nginx VPS (`156.38.222.220`, same box as
    `n8n.titan-ai.co.za`). nginx terminates TLS (Let's Encrypt via
    `certbot --nginx`), serves the static Vite `dist/`, and reverse-proxies
    `/api/*` to uvicorn on `127.0.0.1:8001`. `systemd` supervises uvicorn.
  - **Where it lives:** `deploy/nginx/watch.titan-ai.co.za.conf`;
    `deploy/tender-watch-backend.service`; `deploy/install.sh`;
    `deploy/deploy.sh`; `deploy/README.md`; `CLAUDE.md`.
  - **Note:** unlike the Caddy/galactix move, this nginx/titan-ai pivot has
    no corresponding entry under `docs/superpowers/specs/` or `/plans/` —
    `CLAUDE.md` and `deploy/` are the only authoritative sources for it.
...
- **CORS** (from `docs/superpowers/specs/2026-06-24-drop-vercel-render-design.md` §4.1)
  - **Decision:** Production `TW_ALLOWED_ORIGINS=""`. `create_app(origins=[])`
    ...
    SPA makes same-origin requests because nginx reverse-proxies `/api/*`.
...
- **TLS** (from `docs/superpowers/specs/2026-06-24-drop-vercel-render-design.md` §12, updated for the nginx pivot)
  - **Decision:** nginx + `certbot --nginx` issues and renews a Let's Encrypt
    certificate for `watch.titan-ai.co.za` via ACME HTTP-01 on port 80; a
    `certbot.timer` systemd timer handles renewal.
  - **Where it lives:** `deploy/nginx/watch.titan-ai.co.za.conf`;
    `deploy/README.md` ("TLS renewals").
