...
- **Deploy** (`deploy/`) — Single self-hosted origin at
  ...
  `certbot --nginx`), serves the Vite `dist/`, and reverse-proxies
  `/api/*` to uvicorn on `127.0.0.1:8001`; `systemd` supervises
  uvicorn. `install.sh` is first-time host setup; `deploy.sh` ships
  laptop → host updates. See `deploy/README.md` for the runbook.

Design spec: `docs/superpowers/specs/2026-06-24-drop-vercel-render-design.md`
(self-hosting choice — the move to nginx/xneelo/titan-ai.co.za described
below happened after this spec was written and is not yet captured in a
superpowers spec doc). v0.1.0 design history:
...
| ------------------------- | ------------------------------------ | -------------------------------------------------------- |
| `TW_API_BASE`             | `https://ocds-api.etenders.gov.za`   | eTenders OCDS Public API base URL (was `data.etenders.gov.za`; relocated) |
| `TW_CONFIG_PATH`          | `./config/tender-watch.json`         | Where the user-editable filter / threshold JSON lives (prod: `/etc/tender-watch/config.json`) |
| `TW_CACHE_TTL_SECONDS`    | `60`                                 | In-memory response cache TTL (seconds)                   |
| `TW_ALLOWED_ORIGINS`      | `http://localhost:5173`              | Comma-separated CORS origins (empty in prod — same-origin behind nginx) |
...
| Variable          | Default                  | Purpose                                              |
| ----------------- | ------------------------ | ---------------------------------------------------- |
| `VITE_API_BASE`   | (empty → uses `/api/*`)  | When set, the SPA calls this base URL instead of the Vite dev-server proxy. Leave it empty in both dev (Vite proxy) and prod (same-origin under nginx). |
...
Single-host deployment at `https://watch.titan-ai.co.za`, on the shared
xneelo nginx VPS (`156.38.222.220`, same box as `n8n.titan-ai.co.za`).
nginx terminates TLS, serves the Vite `dist/`, and reverse-proxies
`/api/*` to uvicorn on `127.0.0.1:8001`. `systemd` supervises uvicorn.

- `deploy/nginx/watch.titan-ai.co.za.conf` — nginx vhost config (installed by `install.sh`).
...
```bash
# Add an A record: watch.titan-ai.co.za -> 156.38.222.220
ssh user@<host-ip>
cd <repo-root>
...
See `deploy/README.md` for the full runbook (daily ops, backups, rollback).

## Security & hardening

This is a personal tool intended to live on an unguessable subdomain
of `titan-ai.co.za`. No auth at the application layer — the
subdomain is the only protection. The host is hardened by:

- nginx auto-renewing a Let's Encrypt TLS certificate via `certbot --nginx`
  (Mozilla "A" grade by default).
- uvicorn binding to `127.0.0.1:8001` only — never reachable from
