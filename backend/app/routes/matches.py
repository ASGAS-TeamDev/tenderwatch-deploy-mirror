...
async def get_matches(
    ...
    if force_refresh:
        ...
        # upstream fetch; it's rare (only on config save).
        ...
        response = await _cache.get_or_load_stale(key, loader)
...
async def warm_default_cache() -> MatchesResponse | None:
    """Pre-warm the cache for the current config + lookback window.
    ...
    try:
        ...
        effective_include_closed = config.include_closed
        key = _cache_key(config_digest, window, effective_include_closed)
        ...
        response = await get_matches(
            window=window, include_closed=effective_include_closed, bust=None,
        )
        return response
    except Exception:
